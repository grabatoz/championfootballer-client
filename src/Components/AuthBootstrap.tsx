...
export default function AuthBootstrap() {
  useEffect(() => {
    const restore = () => {
      ...
      // Only restore if we have cookies but missing localStorage data
      if (hasCookie && !haveBasics) {
        const auth = authStorage.getAuth()
        if (auth?.user && auth?.userData && auth?.token) {
          // Use authStorage to ensure consistent storage
          authStorage.saveAuthExact(auth.user, auth.userData, auth.token)
        }
      }
    }
