    ...
    >
      <Box>
        ...
        <Box sx={{ 
          ...
          height: { xs: 45, sm: 60, md: 90 },
          width: { xs: 45, sm: 60, md: 90 },
          margin: '0 auto'
        }}>
          <Image 
            src={image} 
...
export default function TrophyRoom({ leagueId }: { leagueId: string }) {
    ...
    const displayedTrophies = useMemo(() => {
        if (filter === 'all') return calculatedTrophies;
        if (!user) return [];
        const myTrophies = calculatedTrophies.filter(t => t.winnerId && t.winnerId === user.id);
        return myTrophies;
    }, [filter, calculatedTrophies, user]);
    ...
    return (
        <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, minHeight: '100vh' }}>
            ...
            {filter === 'my' && displayedTrophies.length === 0 && (
